...
| `node_position_overrides.csv` | Corrected coordinates for nodes whose `All_nodeslines` rows disagree on position by more than one cell (the run and `hubs validate` list them). Empty template shipped; add a row per conflicting node. | `node, X, Y, notes` (EPSG:2039) | UTF-8 |
| `is_same_group.csv` | Manual hub merges: nodes that must share one group even if farther than 120 m apart | `Nodes in group` — comma-separated node IDs, one row per merge; optional `model` restricts the row to that demand model | UTF-8 |

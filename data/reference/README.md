...
| `h3_base.parquet` (+ `h3_base.manifest.json`) | **Prototype.** The four layers above pre-allocated to H3 resolution-10 cells (1.57 M rows) by `hubs prepare-base`; used when a run sets `spatial_source=h3_base`. The manifest records the source files and their SHA-256. Rebuild after changing any of the four shapefiles. See `docs/H3_BASE_LAYER.md`. | `h3_index, area, location, term_type, term_id, bus_terminal, pop_2050, emp_2050` | Parquet (zstd) |

...
Keep `group` as-is for the page. Add `hub_id = "H" + sha1(sorted node ids)[:10]` (stable while membership is unchanged) written to `OUT/hub_identity.csv` (`group,hub_id,nodes,HubNameHE`). Key reference tables by **node IDs**: `is_same_group.csv` and `manual_demand_updates.csv` already are; migrate `HubsNames_<date>.csv` (`group,HubNameHE`) to `data/reference/hub_names.csv` with `nodes,HubNameHE,notes` via a one-off `scripts/migrate_hub_names.py` (joins the old file to the golden workbook's `group→node`). Lookup: first row whose node set intersects the group's nodes; ambiguous matches go to the report. Legacy `group`-keyed file still accepted with a deprecation warning.
...
1. The four shapefiles (`metro_2008`, `Districts`, `BUS_TERMINAL_STRAT`, `TAZ_1270`) for `data/reference/` and for the full golden test. Until then, the network/scoring/post-processing stages are verified against the golden; the spatial stages only against synthetic fixtures.
2. `HubsNames_28122025.csv` and `IsSameGroup.csv` (current versions) for the migration script and the grouping comparison.
3. Confirmation of the ring radii choice (B5) and, later, a node-coordinates layer if stage 0 (auto-build of `All_nodeslines`) is wanted.

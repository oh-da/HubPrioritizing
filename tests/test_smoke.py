...
def test_missing_required_layer_fails_loudly(dirs):
    ...
    result = run_pipeline(load_config(None, {"mc_iterations": 50}), inputs, allow_missing_layers=True)
...
def test_cli_run(dirs, tmp_path, capsys, monkeypatch):
    ...
    # a missing layer is a hard error for the CLI ...
    for ext in (".shp", ".shx", ".dbf", ".prj", ".cpg"):
    ...
    assert main(["run", "--input-dir", str(input_dir), "--reference-dir", str(ref_dir), "--output-dir", str(out), "--set", "mc_iterations=50"]) == 0
